                    ...
                    <div class="col-xxl-4 col-md-6">
                        <div class="card info-card sales-card">
                            <div class="filter p-2">
                                <input type="month" class="form-control" name="" id="filterByMonth"
                                    value="{{ $carbon->now()->format('Y-m') }}" onchange="monthFilter()">
                            </div>
                            <div class="card-body">
                                <h5 class="card-title">Total Downtime ahayyy</h5>
                                <div class="d-flex align-items-center">
                                    ...
                                    <div class="ps-3">
                                        ...
                                        <span class="text-success small pt-1 fw-bold">{{ $totalMachineRepairs }}</span>
                                        <span class="text-muted small pt-2 ps-1">Mesin down bulan ini</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="card">
                        <div class="card-body">
                            <div class="card-title d-flex justify-content-between">
                                ...
                                <?php if(auth()->user()->username == 'admin' || auth()->user()->username == 'manager' || auth()->user()->username == 'maintenance'): ?>
                                ...
                                <form action="/export/machine-repairs" method="post">
                                    ...
                                    <tbody>
                                        <tr>
                                            ...
                                            <td scope="col"><input type="text" class="form-control" id="minRusak"
                                                    name="min"></td>
                                            <td rowspan="2"><button type="submit"
                                                    class="btn btn-success">Export</button></td>
                                        </tr>
                                        <tr>
                                            <td scope="col">Maximum date:</td>
                                            <td scope="col"><input type="text" class="form-control" id="maxRusak"
                                                    name="max"></td>
                                        </tr>
                                    </tbody>
                                </form>
                            ...
                            <table class="table table-fixed table-bordered table-striped"
                                ...
                                <thead class="mt-4">
                                    <tr>
                                        ...
                                        <?php if(auth()->user()->username == 'admin' || auth()->user()->username == 'manager' || auth()->user()->departement == 'maintenance'): ?>
                                    ...
                                    @foreach ($machineRepairs as $machineRepair)
                                        <tr>
                                            ...
                                            <td id='downtime{{ $machineRepair->id }}' class='bg-success text-light'>
                                                @php
                                                    // Menentukan downtime berdasarkan status aktifitas mesin
                                                    $downtime =
                                                        $machineRepair->status_aktifitas === 'Running'
                                                            ? $machineRepair->total_downtime // Tampilkan downtime jika statusnya "Running"
                                                            : $machineRepair->downtime; // Tampilkan downtime jika statusnya bukan "Running"

                                                    // Mengekstrak bagian-bagian waktu
                                                    $parts = explode(':', $downtime);
                                                    $days = $parts[0] ?? 0;
                                                    $hours = $parts[1] ?? 0;
                                                    $minutes = $parts[2] ?? 0;
                                                    $seconds = $parts[3] ?? 0;

                                                    // Tampilkan downtime hanya jika status mesin "Running"
                                                    if ($machineRepair->status_aktifitas === 'Running') {
                                                        echo "{$days} hari <br> {$hours} jam <br> {$minutes} menit <br> {$seconds} detik";
                                                    } else {
                                                        echo '-'; // Menampilkan tanda "-" jika statusnya "Stop"
                                                    }
                                                @endphp
                                            </td>
